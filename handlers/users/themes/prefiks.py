...
cb_get_prefiks_answers = CallbackData("get_prefiks_answers", "answer_id")


@dp.callback_query_handler(text='prefiks', state="*")
async def get_questions_apostrof(call: types.CallbackQuery, state: FSMContext):
    await call.message.edit_reply_markup()
    await call.message.edit_text("💫Ви обрали тему «Правопис префіксів»💫")
    keyboard = types.InlineKeyboardMarkup(row_width=1)
    questions = await db.select_all_questions_prefiks()
    ...
    for index, variable in enumerate(variables, start=1):
        keyboard.add(types.InlineKeyboardButton(text=variable,
                                                callback_data=cb_get_prefiks_answers.new(answer_id=index)))
    ...
    await Test.Prefiks.set()


@dp.callback_query_handler(cb_get_prefiks_answers.filter(), state=Test.Prefiks)
