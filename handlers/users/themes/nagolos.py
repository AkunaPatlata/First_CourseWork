...
cb_get_nagolos_answers = CallbackData("get_nagolos_answers", "answer_id")


@dp.callback_query_handler(text='nagolos', state="*")
async def get_questions_nagolos(call: types.CallbackQuery, state: FSMContext):
    await call.message.edit_reply_markup()
    await call.message.edit_text("💫Ви обрали тему «Наголос»💫")
    keyboard = types.InlineKeyboardMarkup(row_width=1)
    questions = await db.select_all_questions_nagolos()
    ...
    for index, variable in enumerate(variables, start=1):
        keyboard.add(types.InlineKeyboardButton(text=variable,
                                                callback_data=cb_get_nagolos_answers.new(answer_id=index)))
    ...
    await Test.Nagolos.set()


@dp.callback_query_handler(cb_get_nagolos_answers.filter(), state=Test.Nagolos)
